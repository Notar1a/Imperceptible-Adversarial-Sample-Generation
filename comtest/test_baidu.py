...
def get_score(jsons):
    fcnt, ED_sum = 0, 0
    for json in jsons:
        # 将label和result都转换为小写再进行比较
        label = json["label"].lower()
        result = json["words_result"][0]["words"].lower()

        fcnt += 0 if label == result else 1
        ED_sum += edit_distance(label, result)
        if fcnt != 0:
...
if __name__ == "__main__":
    ...
    # update the data path to point to your perturbed-output directory
    data_path = "../perturbed-output"
    ...
    for imagefile in imagefiles:
        ...
        answer["label"] = imagename.split("_")[1]  # Assuming this format is correct for your labels
        try:
